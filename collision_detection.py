...
class CollisionDetector:
    ...
    def __init__(self, tile_coordinates, dx, dy):
    ...
    def check_board(self, shape_coordinates):
        if np.max(shape_coordinates[:,1]) >= self._board_height:
            return Collision.BOTTOM
        if np.max(shape_coordinates[:,0]) >= self._board_width:
    ...
    def check_ground(self, tile_coordinates, dx, dy):
        ...
        for ground_coord_1 in ground_coordinates:
            for tile_coord_1 in tile_coordinates:
                if np.all(ground_coord_1 == tile_coordinates):
                    if dy > 0:
                        return Collision.BOTTOM
                    if dx > 0:
                        return Collision.RIGHT
